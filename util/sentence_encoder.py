...
class BERTPAIRSentenceEncoder(nn.Module): 
    ...
    def tokenize(self, raw_tokens):
        #print('#####################')
        tokens = []
        raw_tokens=str(raw_tokens).split()
        #print(raw_tokens)
        for token in raw_tokens:
            #print(token)
            token = token.lower()
...
class BERTSentenceEncoder(nn.Module):
    ...
    def __init__(self, pretrain_path, max_length,rep='cls'): 
        nn.Module.__init__(self)
    ...
    def forward(self, inputs):
        x = self.bert(inputs['word'], attention_mask=inputs['mask'])[1]#cls
       
        return x
    
    def tokenize(self, raw_tokens,label_name=None):
        if label_name is None:
            inputs=self.tokenizer(raw_tokens,return_tensors="pt")
        else:
            inputs=self.tokenizer(raw_tokens,label_name,return_tensors="pt")
        
        tokens=inputs['input_ids'].shape[1]

        word = np.zeros((self.max_length), dtype=np.int32)
        word[:min(tokens,self.max_length)]=inputs['input_ids'][0][:min(tokens,self.max_length)]

        mask = np.zeros((self.max_length), dtype=np.int32)
        mask[:min(tokens,self.max_length)]=inputs['attention_mask'][0][:min(tokens,self.max_length)]

        #seg = np.ones((self.max_length), dtype=np.int32)
        #seg[:min(tokens,self.max_length)]=inputs['token_type_ids'][0][:min(tokens,self.max_length)]
